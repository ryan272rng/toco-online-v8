...
const CardFanIcon = () => (
  <svg width="34" height="34" viewBox="-2 -2 38 38" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg opacity-90 mb-1">
    <rect x="2" y="2.5" width="16" height="23" rx="3" fill="white" stroke="#D1D5DB" strokeWidth="1.5" />
    <text x="4" y="9" fill="#DC2626" fontFamily="sans-serif" fontSize="5" fontWeight="bold">♥</text>
    <rect x="0" y="4.5" width="16" height="23" rx="3" transform="rotate(-15 0 4.5)" fill="white" stroke="#D1D5DB" strokeWidth="1.5" />
    <text x="1" y="9" fill="#111827" fontFamily="sans-serif" fontSize="5" fontWeight="bold" transform="rotate(-15 1 9)">♣</text>
    <rect x="12" y="2.5" width="16" height="23" rx="3" transform="rotate(15 12 2.5)" fill="white" stroke="#D1D5DB" strokeWidth="1.5" />
    <text x="13" y="8" fill="#DC2626" fontFamily="sans-serif" fontSize="5" fontWeight="bold" transform="rotate(15 13 8)">♦</text>
    <rect x="18" y="4.5" width="16" height="23" rx="3" transform="rotate(30 18 4.5)" fill="white" stroke="#D1D5DB" strokeWidth="1.5" />
    <text x="19" y="11" fill="#111827" fontFamily="sans-serif" fontSize="5" fontWeight="bold" transform="rotate(30 19 11)">♠</text>
  </svg>
);

const SUITS = {
  hearts: { symbol: "♥", defaultColor: "text-red-600", darkColor: "text-red-400", luxoColor: "text-red-500", name: "Copas" },
  diamonds: { symbol: "♦", defaultColor: "text-red-600", darkColor: "text-red-400", luxoColor: "text-red-500", name: "Ouros" },
  clubs: { symbol: "♣", defaultColor: "text-slate-900", darkColor: "text-white", luxoColor: "text-yellow-400", name: "Paus" },
  spades: { symbol: "♠", defaultColor: "text-slate-900", darkColor: "text-white", luxoColor: "text-yellow-400", name: "Espadas" },
};
...
const getTableBg = (style) => `bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] ${TABLE_STYLES[style] || TABLE_STYLES.tradicional}`;
...
const PlayerAvatar = ({ src, name = "", size = "md", isTurn, isOpponent }) => {
  ...
  const dim = size === "lg" ? "w-24 h-24 text-3xl" : size === "sm" ? "w-10 h-10 text-sm" : "w-10 h-10 md:w-12 md:h-12 text-lg";
  ...
  if (src) {
    return (
      <img src={src} alt={safeName} className={`${dim} rounded-full object-cover ${ringColor} ${turnScale} transition-all duration-300`} />
    );
  }
  return (
    <div className={`${dim} rounded-full bg-gradient-to-br from-blue-700 to-indigo-900 ${ringColor} ${turnScale} flex items-center justify-center text-white font-black transition-all duration-300`}>
      {safeName.includes("Robô") || safeName === "Computador" ? "🤖" : initial}
    </div>
  );
};

const CardFace = ({ card, playable, onClick, isHinted, trumpSuit, localProcessing, settings }) => {
  ...
  const hintClass = isHinted ? "ring-4 ring-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.8)] -translate-y-4 scale-105 z-20" : "";
  ...
  const sizeClasses = cardSize === "large" ? "w-[84px] h-[120px] md:w-28 md:h-40" : "w-[72px] h-[104px] md:w-24 md:h-36";

  const renderCenter = () => {
    if (card.label === "K") return <div className="text-4xl md:text-5xl drop-shadow-sm">🤴</div>;
    if (card.label === "Q") return <div className="text-4xl md:text-5xl drop-shadow-sm">👸</div>;
    if (card.label === "J") return <div className="text-4xl md:text-5xl drop-shadow-sm">💂</div>;
    ...
    if (!isNaN(num)) {
      ...
      return (
        <div className={`grid ${grid} gap-1 items-center justify-items-center h-full w-full px-1.5 ${cardTextColor}`}>
          {Array.from({ length: num }).map((_, i) => (
            <span key={i} className={`text-base md:text-lg leading-none ${i >= Math.ceil(num / 2) ? "rotate-180" : ""}`}>{suitDef.symbol}</span>
          ))}
        </div>
      );
    }
    return <div className={`text-5xl md:text-6xl drop-shadow-sm font-sans ${cardTextColor}`}>{suitDef.symbol}</div>;
  };
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none text-8xl md:text-[100px] overflow-hidden font-sans ${cardTextColor}`}>{suitDef.symbol}</div>
      <div className={`absolute top-1 left-1.5 flex flex-col items-center justify-center ${cardTextColor} z-10`}>
        <span className="font-bold text-[12px] md:text-sm tracking-tighter leading-none">{card.label}</span>
        <span className="text-[11px] md:text-xs mt-[2px] leading-none font-sans">{suitDef.symbol}</span>
      </div>
      <div className="flex-1 flex items-center justify-center w-full mt-3 mb-2 px-1 z-10">{renderCenter()}</div>
      <div className={`absolute bottom-1 right-1.5 flex flex-col items-center justify-center rotate-180 ${cardTextColor} z-10`}>
        <span className="font-bold text-[12px] md:text-sm tracking-tighter leading-none">{card.label}</span>
        <span className="text-[11px] md:text-xs mt-[2px] leading-none font-sans">{suitDef.symbol}</span>
      </div>
      {isTrump && (
...
const MiniCard = ({ card, settings }) => {
  ...
  return (
    <div className={`relative w-10 h-14 ${bgClass} rounded border shadow-sm flex flex-col items-center justify-center p-1 overflow-hidden`}>
      <div className={`absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none text-4xl font-sans ${textColor}`}>{suitDef.symbol}</div>
      <span className={`text-xs font-bold leading-none z-10 ${textColor}`}>{card.label}</span>
      <span className={`text-xl z-10 font-sans ${textColor}`}>{suitDef.symbol}</span>
    </div>
  );
};
...
const CardBack = ({ settings, isMini = false }) => {
  ...
  const sizeClasses = isMini 
    ...
    : (cardSize === "large" ? "w-[84px] h-[120px] md:w-28 md:h-40 border-2" : "w-[72px] h-[104px] md:w-24 md:h-36 border-2");
  ...
  return (
    ...
    >
      ...
      <div className={`${isMini ? 'w-4 h-6' : 'w-8 h-12 md:w-10 md:h-16'} border-2 border-white/30 rounded-lg flex items-center justify-center bg-blue-800/80`}>
...
const EdgePlayer = ({ player, handCount, position, isTurn, isOpponent, tocoTarget, lives, settings }) => {
  if (!player) return null;

  let containerClass = "absolute flex flex-col items-center z-10 transition-all duration-300 ";
  ...
  if (position === "top") {
    ...
    containerClass += "left-2 md:left-6 top-[60%] -translate-y-1/2 flex-row gap-4";
    flexDir = "flex-col items-start";
    infoAlign = "text-left";
  } else if (position === "right") {
    containerClass += "right-2 md:right-6 top-[60%] -translate-y-1/2 flex-row-reverse gap-4";
  ...
  let fanGlow = "";
  if (isTurn) {
    if (isOpponent) fanGlow = "drop-shadow-[0_0_25px_rgba(239,68,68,1)] scale-110 z-20";
  ...
  return (
    <div className={containerClass}>
      <div className={`flex ${flexDir} items-center gap-1 text-center`}>
        <PlayerAvatar src={player.avatar} name={player.name} size="sm" isTurn={isTurn} isOpponent={isOpponent} />
        <div className={`bg-black/50 backdrop-blur rounded px-2 py-0.5 text-white/90 flex flex-col ${infoAlign} gap-0.5 shadow-md border border-white/10 ${isTurn ? 'scale-110 z-20' : ''} transition-transform`}>
          <span className="text-[10px] md:text-xs font-bold block truncate max-w-[70px]">{player.name}</span>
        </div>
      </div>

      {handCount > 0 && (
        <div className={`relative flex ${position === "top" ? "flex-row -space-x-6" : "flex-col -space-y-8"} items-center justify-center transition-all duration-500 opacity-90 ${fanGlow} ${position === "top" ? "mt-1" : ""}`}>
          {Array.from({ length: handCount }).map((_, i) => {
            ...
            if (position === "top") {
              transformStyle = `rotate(${angle}deg) translateY(${Math.abs(offset) * 4}px)`;
            } else if (position === "left") {
              transformStyle = `rotate(${90 + angle}deg) translateX(${-Math.abs(offset) * 4}px)`;
            } else if (position === "right") {
              transformStyle = `rotate(${-90 + angle}deg) translateX(${Math.abs(offset) * 4}px)`;
            }

            return (
              <div key={i} style={{ transform: transformStyle, transformOrigin: "center center" }} className="transition-all duration-300">
...
export default function App() {
  // ============================================================================
  // 2. ESTADOS GERAIS E CONFIGURAÇÕES BLINDADOS
  // ============================================================================
  const [playerName, setPlayerName] = useState(() => {
    try { return localStorage.getItem("tocoPlayerName") || ""; } catch(e) { return ""; }
  });
  ...
  const [avatarBase64, setAvatarBase64] = useState(() => {
    try { return localStorage.getItem("tocoPlayerAvatar") || ""; } catch(e) { return ""; }
  });
  ...
  const [settings, setSettings] = useState(() => {
    try {
      ...
      return { sound: true, showHints: false, deckStyle: "default", cardSize: "normal", animations: true, tableStyle: "tradicional", bgm: true };
    } catch(e) {
      return { sound: true, showHints: false, deckStyle: "default", cardSize: "normal", animations: true, tableStyle: "tradicional", bgm: true };
    }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem("tocoSettings", JSON.stringify(settings)); } catch(e){}
  }, [settings]);

  useEffect(() => {
    try { if (playerName.trim()) localStorage.setItem("tocoPlayerName", playerName); } catch(e){}
  }, [playerName]);
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        try { localStorage.setItem("tocoPlayerAvatar", compressedBase64); } catch(e){}
      };
  ...
  const playSoundEffect = (type) => {
    ...
    try {
      ...
      if (type === "card") {
        osc.type = "sine"; osc.frequency.setValueAtTime(400, ctx.currentTime); osc.frequency.exponentialRampToValueAtTime(50, ctx.currentTime + 0.1); gain.gain.setValueAtTime(0.5, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.1); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.1);
      } else if (type === "heavy_card") {
        osc.type = "square"; osc.frequency.setValueAtTime(150, ctx.currentTime); osc.frequency.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.3); gain.gain.setValueAtTime(1, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.3); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.3);
      } else if (type === "win") {
        osc.type = "triangle"; osc.frequency.setValueAtTime(440, ctx.currentTime); osc.frequency.setValueAtTime(554, ctx.currentTime + 0.1); osc.frequency.setValueAtTime(659, ctx.currentTime + 0.2); gain.gain.setValueAtTime(0.3, ctx.currentTime); gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.5); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.5);
      } else if (type === "lose") {
        osc.type = "sawtooth"; osc.frequency.setValueAtTime(250, ctx.currentTime); osc.frequency.linearRampToValueAtTime(100, ctx.currentTime + 0.4); gain.gain.setValueAtTime(0.3, ctx.currentTime); gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.4); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.4);
      } else if (type === "trick_win") {
        osc.type = "sine"; osc.frequency.setValueAtTime(523.25, ctx.currentTime); osc.frequency.setValueAtTime(659.25, ctx.currentTime + 0.1); gain.gain.setValueAtTime(0.3, ctx.currentTime); gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.2); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.2);
      } else if (type === "trick_lose") {
        osc.type = "triangle"; osc.frequency.setValueAtTime(300, ctx.currentTime); osc.frequency.setValueAtTime(200, ctx.currentTime + 0.1); gain.gain.setValueAtTime(0.3, ctx.currentTime); gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.2); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.2);
      }
  ...
  useEffect(() => { stateRef.current = roomData; }, [roomData]);
  useEffect(() => { setCurrentHint(null); }, [roomData?.tableCards?.length, roomData?.turn]);
  ...
  const createRoom = async () => {
    ...
    const newMe = { id: myId, name: playerName, isHost: true, avatar: avatarBase64, slot: 0 };

    const initialRoomData = {
      gameState: "lobby", hostId: myId, gameMode: selectedMode, players: { [myId]: newMe }, deck: [], tableCards: [], trumpSuit: null, turn: null, hands: {}, roundScores: { [myId]: 0 }, gamePoints: { [myId]: 0 }, tocoTarget: null, lives: 3, trickHistory: [], sweepingTo: null
    };
  ...
  const joinRoom = async () => {
    if (isNetworkOffline) return setErrorMsg("Conecte-se à internet.");
    if (!playerName.trim()) return setErrorMsg("Digite seu nome primeiro!");
    if (!pinInput.trim() || pinInput.length !== 4) return setErrorMsg("PIN inválido!");
    if (isJoining) return;
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        const matchedPlayer = existingPlayers.find((p) => String(p?.name || "").toLowerCase() === playerName.trim().toLowerCase());
        ...
        if (matchedPlayer) {
          ...
          newMe = matchedPlayer;
          if (avatarBase64) await update(ref(database, `rooms/${pinInput}/players/${myId}`), { avatar: avatarBase64 });
        } else if (existingPlayers.length >= maxPlayers) {
          setErrorMsg("A sala já está cheia!");
          setIsJoining(false);
          return;
        } else {
          const occupiedSlots = existingPlayers.map((p) => p?.slot !== undefined ? p.slot : -1);
          let freeSlot = Array.from({ length: maxPlayers }, (_, i) => i).find((s) => !occupiedSlots.includes(s));
          if (freeSlot === undefined) freeSlot = existingPlayers.length;

          myId = `player_${Date.now()}`;
          newMe = { id: myId, name: playerName, isHost: false, avatar: avatarBase64, slot: freeSlot };
          await update(ref(database, `rooms/${pinInput}/players`), { [myId]: newMe });
          await update(ref(database, `rooms/${pinInput}/roundScores`), { [myId]: 0 });
          await update(ref(database, `rooms/${pinInput}/gamePoints`), { [myId]: 0 });
        }

        // PROTEÇÃO 1: Se o jogador que entrou fechar a aba, remove ele da sala
        onDisconnect(ref(database, `rooms/${pinInput}/players/${myId}`)).remove();
  ...
  const changeSlot = (newSlot) => {
    ...
    update(ref(database, `rooms/${roomId}/players/${me.id}`), { slot: newSlot });
  ...
  const startSinglePlayer = () => {
    ...
    if (selectedMode === "2v2") {
      bots = {
        "bot_1": { id: "bot_1", name: "Robô Esquerda", isHost: false, avatar: "", slot: 1 },
        "bot_2": { id: "bot_2", name: "Robô Aliado", isHost: false, avatar: "", slot: 2 },
        "bot_3": { id: "bot_3", name: "Robô Direita", isHost: false, avatar: "", slot: 3 }
      };
      initialScores = { ...initialScores, bot_1: 0, bot_2: 0, bot_3: 0 };
      initialPoints = { ...initialPoints, bot_1: 0, bot_2: 0, bot_3: 0 };
    } else {
      bots = {
        "bot_1": { id: "bot_1", name: "Computador", isHost: false, avatar: "", slot: 1 }
      };
    ...
    const initialRoomData = {
      gameState: "choose_trump", hostId: myId, gameMode: selectedMode, players: { [myId]: newMe, ...bots }, deck: createDeepShuffleDeck(), tableCards: [], trumpSuit: null, turn: null, hands: {}, roundScores: initialScores, gamePoints: initialPoints, tocoTarget: myId, lives: 3, trickHistory: [], sweepingTo: null
    };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      ...
      else { setErrorMsg("A sala foi fechada pelo Host."); setRoomId(null); }
    });
    return () => unsubscribe();
  ...
  const playersList = roomData?.players ? Object.values(roomData.players).filter(Boolean).sort((a, b) => (a.slot || 0) - (b.slot || 0)) : [];
  ...
  const myIdx = playersList.findIndex((p) => p?.id === me?.id) !== -1 ? playersList.findIndex((p) => p?.id === me?.id) : 0;
  const is2v2 = gameMode === "2v2";

  const myTeam = is2v2 ? [playersList[myIdx], playersList[(myIdx + 2) % 4]].filter(Boolean) : [playersList[myIdx]].filter(Boolean);
  const opTeam = is2v2 ? [playersList[(myIdx + 1) % 4], playersList[(myIdx + 3) % 4]].filter(Boolean) : [playersList[(myIdx + 1) % 2]].filter(Boolean);

  const leftPlayer = is2v2 ? playersList[(myIdx + 1) % 4] : null;
  const topPlayer = is2v2 ? playersList[(myIdx + 2) % 4] : playersList[(myIdx + 1) % 2];
  const rightPlayer = is2v2 ? playersList[(myIdx + 3) % 4] : null;

  const myTeamScore = myTeam.reduce((acc, p) => acc + (roundScores[p?.id] || 0), 0);
  const opTeamScore = opTeam.reduce((acc, p) => acc + (roundScores[p?.id] || 0), 0);
  const myTeamTocos = myTeam.reduce((acc, p) => acc + (gamePoints[p?.id] || 0), 0);
  const opTeamTocos = opTeam.reduce((acc, p) => acc + (gamePoints[p?.id] || 0), 0);

  const getCardPower = (card, currentTrump, leadSuit) => {
    const basePower = { A: 100, 7: 90, K: 80, J: 70, Q: 60, 6: 50, 5: 40, 4: 30, 3: 20, 2: 10 };
  ...
  const checkIfHeavy = (playedCard, currentTable, trump) => {
    if (currentTable.length === 0) {
      return ["A", "7"].includes(playedCard.label) || (playedCard.suit === trump && ["3", "2"].includes(playedCard.label));
    }
  ...
  const getBestCardToPlay = (playerId, currentHand, currentTable, currentTrump, currentScore) => {
    const isFirstToPlay = currentTable.length === 0;
    const myTrumps = currentHand.filter((card) => card.suit === currentTrump).sort((a, b) => getCardPower(a, currentTrump, currentTrump) - getCardPower(b, currentTrump, currentTrump));
    const lowTrumps = myTrumps.filter((card) => ["Q", "J", "K", "4", "5", "6"].includes(card.label));
    const sortedHandByPtsAsc = [...currentHand].sort((a, b) => getCardPoints(a, currentTrump) - getCardPoints(b, currentTrump));
    ...
    if (!isFirstToPlay) {
      ...
      for (let card of currentHand) {
        ...
        if (cardPower > maxTablePower && currentScore + tablePoints + cardPts >= POINTS_GOAL) return card;
      }
    }
    ...
    if (isFirstToPlay) {
      ...
      const winningCards = currentHand.filter((c) => getCardPower(c, currentTrump, leadSuit) > maxTablePower);
      const winningLeadSuit = winningCards.filter((c) => c.suit === leadSuit);
      const winningTrumps = winningCards.filter((c) => c.suit === currentTrump);

      if (winningLeadSuit.length > 0 && (opPts > 0 || getCardPoints(winningLeadSuit[0], currentTrump) > 0)) {
        return [...winningLeadSuit].sort((a, b) => getCardPoints(b, currentTrump) - getCardPoints(a, currentTrump))[0];
  ...
  const generateHint = () => {
    ...
    const bestCard = getBestCardToPlay(me.id, myHand, tableCards, trumpSuit, myTeamScore);
    if (!bestCard) return;

    let explanation = "";
    const isFirstToPlay = tableCards.length === 0;

    if (!isFirstToPlay && getCardPower(bestCard, trumpSuit, tableCards[0].card.suit) > getCardPower(tableCards[0].card, trumpSuit, tableCards[0].card.suit) && myTeamScore + getCardPoints(tableCards[0].card, trumpSuit) + getCardPoints(bestCard, trumpSuit) >= POINTS_GOAL) {
      explanation = "Jogue esta! Você vai chegar aos 31 pontos e ganhar a partida agora!";
    } else if (isFirstToPlay) {
      if (bestCard.suit === trumpSuit) explanation = "Saia cortando baixo para forçar os oponentes a gastarem trunfos altos.";
      else if (getCardPoints(bestCard, trumpSuit) === 0) explanation = "Jogue um Limpo para ver a reação deles.";
      else explanation = "Você não tem Limpos. Saia com a carta de menor valor para que o prejuízo seja pequeno.";
    } else {
      const leadSuit = tableCards[0].card.suit;
      if (bestCard.suit === leadSuit && getCardPower(bestCard, trumpSuit, leadSuit) > getCardPower(tableCards[0].card, trumpSuit, leadSuit)) {
        if (leadSuit === trumpSuit) explanation = "Corte! Você tem um trunfo maior. Roube os pontos da mesa.";
        else explanation = "Encarte! Jogue uma carta maior do mesmo naipe e garanta os pontos.";
      } else if (bestCard.suit === trumpSuit && getCardPower(bestCard, trumpSuit, leadSuit) > getCardPower(tableCards[0].card, trumpSuit, leadSuit)) {
        explanation = "Corte! As cartas deles valem pontos. Use seu trunfo baixo para roubar.";
      } else {
        if (getCardPoints(bestCard, trumpSuit) === 0) explanation = "Não vale a pena gastar carta boa. Jogue um Limpo para não dar pontos a eles.";
        else explanation = "Descarte a sua carta de MENOR valor para diminuir o prejuízo.";
      }
    }
  ...
  useEffect(() => {
    ...
    if (bot && bot.id !== me?.id && tableCards.length < playersList.length) {
      const timer = setTimeout(() => {
        ...
        const cPlayersList = Object.values(currentRoom.players).filter(Boolean).sort((a, b) => (a.slot || 0) - (b.slot || 0));
        const cMyIdx = cPlayersList.findIndex((p) => p?.id === me?.id) !== -1 ? cPlayersList.findIndex((p) => p?.id === me?.id) : 0;
        
        const is2v2Mode = currentRoom.gameMode === "2v2";
        const cMyTeam = is2v2Mode ? [cPlayersList[cMyIdx], cPlayersList[(cMyIdx + 2) % 4]].filter(Boolean) : [cPlayersList[cMyIdx]].filter(Boolean);
        const cOpTeam = is2v2Mode ? [cPlayersList[(cMyIdx + 1) % 4], cPlayersList[(cMyIdx + 3) % 4]].filter(Boolean) : [cPlayersList[(cMyIdx + 1) % 2]].filter(Boolean);
        ...
        const cMyTeamScore = cMyTeam.reduce((acc, p) => acc + (currentRoom.roundScores[p?.id] || 0), 0);
        const cOpTeamScore = cOpTeam.reduce((acc, p) => acc + (currentRoom.roundScores[p?.id] || 0), 0);

        const botScore = isMyTeamBot ? cMyTeamScore : cOpTeamScore;
        const cardToPlay = getBestCardToPlay(botId, botHand, currentRoom.tableCards || [], currentRoom.trumpSuit, botScore);

        if (cardToPlay) {
          const isHeavy = checkIfHeavy(cardToPlay, currentRoom.tableCards || [], currentRoom.trumpSuit);
          const newHand = botHand.filter((c) => c.id !== cardToPlay.id);
          const newTable = [...(currentRoom.tableCards || []), { playerId: botId, card: cardToPlay, isHeavy }];
          ...
          syncState({ hands: { ...currentHands, [botId]: newHand }, tableCards: newTable, turn: nextTurn });
        }
      }, 1800);
      return () => clearTimeout(timer);
    }
  ...
  const createDeepShuffleDeck = () => {
    ...
    Object.keys(SUITS).forEach((suitKey) => {
      RANKS.forEach((rank) => {
        newDeck.push({ ...rank, suit: suitKey, id: `${suitKey}-${rank.label}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}` });
      });
    });
  ...
  const startGameFirstTime = () => {
    ...
    if (playersList.length < requiredPlayers) return setErrorMsg(`Aguardando mais jogadores para ${gameMode}...`);

    const pts = {};
    playersList.forEach((p) => (pts[p.id] = 0));
    const randomStartId = playersList[Math.floor(Math.random() * playersList.length)].id;
  ...
  const startNewHand = (targetId) => {
    ...
    syncState({ deck: createDeepShuffleDeck(), roundScores: rScores, tableCards: [], trickFeedback: null, roundResult: null, trickHistory: [], hands: {}, gameState: "choose_trump", sweepingTo: null });
  ...
  useEffect(() => {
    if (me?.isHost && gameState === "dealing" && roomData) {
      ...
      playersList.forEach((p, index) => {
        if (p?.id) newHands[p.id] = currentDeck.slice(index * 4, (index + 1) * 4);
      });
      const remaining = currentDeck.slice(playersList.length * 4);

      syncState({ hands: newHands, deck: remaining, turn: roomData.tocoTarget, gameState: "announcing_trump" });
  ...
  const handleCardClick = (card) => {
    const currentRoom = stateRef.current;
    if (!me?.id || currentRoom.gameState !== "playing" || currentRoom.turn !== me.id || localProcessing) return;
    
    const cPlayersList = Object.values(currentRoom.players).filter(Boolean).sort((a, b) => (a.slot || 0) - (b.slot || 0));
    if ((currentRoom.tableCards || []).length >= cPlayersList.length) return;

    if (settings.vibration && navigator.vibrate && !checkIfHeavy(card, currentRoom.tableCards || [], currentRoom.trumpSuit)) navigator.vibrate(40);
    ...
    const isHeavy = checkIfHeavy(card, currentRoom.tableCards || [], currentRoom.trumpSuit);
    const newTable = [...(currentRoom.tableCards || []), { playerId: me.id, card, isHeavy }];

    const currentTurnIdx = cPlayersList.findIndex((p) => p?.id === currentRoom.turn);
    ...
    syncState({ hands: { ...currentHands, [me.id]: newHand }, tableCards: newTable, turn: nextTurn });
  };
  ...
  useEffect(() => {
    if (me?.isHost && tableCards.length > 0 && playersList.length > 0 && tableCards.length === playersList.length) {
  ...
  const resolveRound = () => {
    ...
    const winnerName = cPlayersList.find((p) => p.id === winnerId)?.name || "Alguém";
    const trickData = { id: Date.now(), winnerId: winnerId, pts: pts, cards: cards.map((c) => c.card) };

    // PROTEÇÃO 4: Fallback caso falte alguém na equipe (ex: desconexão)
    const destinationId = isMyTeamWin ? me?.id : (cOpTeam[0]?.id || me?.id); 

    syncState({ roundScores: newScores, trickFeedback: { winnerName, pts }, trickHistory: [...currentHistory, trickData], sweepingTo: destinationId });

    const updatedTeamAScore = cMyTeam.reduce((acc, p) => acc + (newScores[p?.id] || 0), 0);
    const updatedTeamBScore = cOpTeam.reduce((acc, p) => acc + (newScores[p?.id] || 0), 0);
      ...
      setTimeout(() => {
        ...
        cards.forEach((tc) => {
          if (freshDeck.length > 0) {
            ...
            if (freshHands[tc.playerId]) freshHands[tc.playerId] = [...(freshHands[tc.playerId] || []), c];
          }
        });

        syncState({ hands: freshHands, deck: freshDeck, turn: winnerId, tableCards: [], sweepingTo: null });
      }, 800);
  ...
  const handleGameEnd = (winningTeam, currentRoom, cPlayersList, cMyTeam, cOpTeam) => {
    const { tocoTarget: currentTarget, lives: currentLives, gamePoints: currentGP } = currentRoom;

    const currentTargetIdx = cPlayersList.findIndex((p) => p.id === currentTarget);
    const targetIsInWinningTeam = winningTeam.some((p) => p?.id === currentTarget);
    ...
    else { playSoundEffect("lose"); if (settings.sound && navigator.vibrate) navigator.vibrate([100, 50, 100]); }
    ...
    if (targetIsInWinningTeam) {
      let nextOpIdx = (currentTargetIdx + 1) % safePlayerLength;
      resultType = "escaped";
      updates = { tocoTarget: cPlayersList[nextOpIdx]?.id || cPlayersList[0]?.id, lives: 3 };
    } else {
      ...
      if (newLives > 0) {
        ...
        let partnerIdx = (currentTargetIdx + 2) % safePlayerLength;
        updates = { gamePoints: gPoints, lives: 3, tocoTarget: cPlayersList[partnerIdx]?.id || cPlayersList[0]?.id };
      }
    }

    updates.roundResult = { type: resultType, isMyTeamWinner, winnerId: winnerRepId, loserId: loserRepId, tocoTargetName: cPlayersList[currentTargetIdx]?.name };
  ...
  const getEndGameMessage = () => {
    ...
    const { type, isMyTeamWinner, tocoTargetName } = roundResult;

    if (type === "escaped") {
      if (isMyTeamWinner) return <span className="text-green-400 drop-shadow-md">O ALVO ({tocoTargetName}) SE LIVROU! 😅</span>;
      else return <span className="text-yellow-400 drop-shadow-md">ELES ESCAPARAM! O TOCO É {is2v2 ? "DE VOCÊS" : "SEU"}! 🫵</span>;
    }
    if (type === "life_lost") {
      if (!isMyTeamWinner) return <span className="text-red-400 drop-shadow-md">ALVO ATINGIDO! {is2v2 ? "SEU TIME" : "VOCÊ"} PERDEU 1 VIDA! 💔</span>;
      else return <span className="text-green-400 drop-shadow-md">ALVO ATINGIDO! VOCÊS TIRARAM 1 VIDA DELES! ⚔️</span>;
    }
    if (type === "toco_confirmed") {
      if (!isMyTeamWinner) return <span className="text-red-600 drop-shadow-md">QUE PENA! O ALVO PEGOU O TOCO. 🪵</span>;
      else return <span className="text-yellow-400 drop-shadow-md">AÊ! {is2v2 ? "VOCÊS DERAM" : "VOCÊ DEU"} UM TOCO NELES! 🏆</span>;
    }
    return null;
  };
  ...
  const LobbySlot = ({ slotNum }) => {
    ...
    return (
      <button onClick={() => changeSlot(slotNum)} disabled={isJoining} className="bg-white/5 border border-dashed border-white/20 px-3 py-3 rounded-lg text-[10px] uppercase font-bold text-gray-400 hover:text-white hover:bg-white/10 w-full transition-colors flex justify-center disabled:opacity-50">Sentar Aqui</button>
    );
  };
    ...
    <div className="absolute inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-gray-900 border border-blue-500/30 rounded-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-6 shadow-[0_0_50px_rgba(59,130,246,0.2)]">
        <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4 sticky top-0 bg-gray-900 z-10">
          <h2 className="text-2xl font-black text-blue-400 flex items-center gap-2">📖 Cartilha do Toco</h2>
          <button onClick={() => setShowRules(false)} className="text-white bg-red-600 rounded-full w-8 h-8 font-bold hover:bg-red-500 transition-colors">&times;</button>
        </div>
        <div className="flex flex-col gap-6 text-sm text-gray-200">
          <section>
            <h3 className="text-lg font-bold text-yellow-400 mb-2 border-l-4 border-yellow-500 pl-2">🎯 O Objetivo</h3>
            <p>O objetivo de cada rodada é ser o primeiro a alcançar <strong>31 pontos</strong>. Ao fazer isso, você dá um "Toco" e tira uma vida (❤️) do adversário. Se ele perder as 3 vidas, o Toco é confirmado!</p>
          </section>

          <section>
            <h3 className="text-lg font-bold text-yellow-400 mb-2 border-l-4 border-yellow-500 pl-2">🔢 Valor das Cartas (Pontos)</h3>
            <div className="grid grid-cols-5 gap-2 text-center font-bold">
              <div className="bg-white/10 rounded p-2"><span className="text-xl block">A</span> 11 pts</div>
              <div className="bg-white/10 rounded p-2"><span className="text-xl block">7</span> 10 pts</div>
              <div className="bg-white/10 rounded p-2"><span className="text-xl block">K</span> 4 pts</div>
              <div className="bg-white/10 rounded p-2"><span className="text-xl block">J</span> 3 pts</div>
              <div className="bg-white/10 rounded p-2"><span className="text-xl block">Q</span> 2 pts</div>
            </div>
            <p className="mt-2 text-xs text-gray-400">*Cartas 6, 5, 4, 3 e 2 (fora do trunfo) são "limpas" e valem 0 pontos.</p>
          </section>

          <section>
            <h3 className="text-lg font-bold text-yellow-400 mb-2 border-l-4 border-yellow-500 pl-2">✨ O Poder do Trunfo</h3>
            <p className="mb-2">Quando o naipe é o Trunfo da rodada, as cartas baixas ganham superpoderes e passam a valer pontos:</p>
            <div className="grid grid-cols-5 gap-2 text-center font-bold">
              <div className="bg-blue-900/40 border border-blue-500/30 rounded p-2"><span className="text-xl block text-blue-400">3</span> 10 pts</div>
              <div className="bg-blue-900/40 border border-blue-500/30 rounded p-2"><span className="text-xl block text-blue-400">2</span> 10 pts</div>
              <div className="bg-blue-900/40 border border-blue-500/30 rounded p-2"><span className="text-xl block text-blue-400">4</span> 4 pts</div>
              <div className="bg-blue-900/40 border border-blue-500/30 rounded p-2"><span className="text-xl block text-blue-400">5</span> 3 pts</div>
              <div className="bg-blue-900/40 border border-blue-500/30 rounded p-2"><span className="text-xl block text-blue-400">6</span> 2 pts</div>
            </div>
          </section>

          <section>
            <h3 className="text-lg font-bold text-yellow-400 mb-2 border-l-4 border-yellow-500 pl-2">⚔️ Hierarquia de Força</h3>
            <p className="mb-1 text-xs text-gray-400">Quem ganha a mão? (Da mais forte para a mais fraca)</p>
            <div className="bg-black/50 p-3 rounded-lg border border-white/5">
              <p className="mb-2"><strong className="text-white">Naipe Normal:</strong><br/> A &gt; 7 &gt; K &gt; J &gt; Q &gt; 6 &gt; 5 &gt; 4 &gt; 3 &gt; 2</p>
              <p><strong className="text-blue-400">No Trunfo:</strong><br/> A &gt; 3 &gt; 7 &gt; 2 &gt; K &gt; 4 &gt; J &gt; 5 &gt; Q &gt; 6</p>
            </div>
          </section>
        </div>
      </div>
    ...
    <div className="absolute inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-gray-900 border border-yellow-500/30 rounded-3xl w-full max-w-sm p-6 shadow-[0_0_50px_rgba(234,179,8,0.2)]">
        <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
          ...
          <button onClick={() => setShowSettings(false)} className="text-white bg-red-600 rounded-full w-8 h-8 font-bold">&times;</button>
        </div>
        <div className="flex flex-col gap-4">

          <label className="flex justify-between items-center text-white font-medium"><span>💡 Exibir botão de Dicas</span><input type="checkbox" checked={settings.showHints} onChange={() => toggleSetting("showHints")} className="w-6 h-6 accent-yellow-500" /></label>
          <label className="flex justify-between items-center text-white font-medium"><span>🔊 Efeitos Sonoros</span><input type="checkbox" checked={settings.sound} onChange={() => toggleSetting("sound")} className="w-6 h-6 accent-yellow-500" /></label>
          <label className="flex justify-between items-center text-white font-medium"><span>🎵 Música de Fundo (Jazz)</span><input type="checkbox" checked={settings.bgm} onChange={() => toggleSetting("bgm")} className="w-6 h-6 accent-yellow-500" /></label>
          ...
          <label className="flex justify-between items-center text-white font-medium border-t border-white/10 pt-4">
            ...
            <input type="checkbox" checked={settings.animations ?? true} onChange={() => toggleSetting("animations")} className="w-6 h-6 accent-yellow-500" />
          </label>

          <div className="text-white font-medium pt-3 border-t border-white/10 mt-2">
            <span className="mb-2 block">🎨 Estilo da Mesa</span>
            <select value={settings.tableStyle || "tradicional"} onChange={(e) => updateSetting("tableStyle", e.target.value)} className="w-full bg-black/50 border border-white/20 rounded p-2 text-sm focus:border-yellow-500 text-white">
          ...
          <div className="text-white font-medium pt-1">
            ...
            <select value={settings.cardSize} onChange={(e) => updateSetting("cardSize", e.target.value)} className="w-full bg-black/50 border border-white/20 rounded p-2 text-sm focus:border-yellow-500 text-white">
          ...
          <div className="text-white font-medium pt-1">
            ...
            <select value={settings.deckStyle} onChange={(e) => updateSetting("deckStyle", e.target.value)} className="w-full bg-black/50 border border-white/20 rounded p-2 text-sm focus:border-yellow-500 text-white">
              <option value="default">Padrão (Branco)</option>
              <option value="dark">Modo Dark (Noturno)</option>
              <option value="luxo">Cassino (Luxo)</option>
            </select>
          </div>
          {roomId && <button onClick={exitGame} className="w-full mt-4 bg-red-600/90 text-white font-bold py-3 rounded-xl hover:bg-red-500 transition-colors uppercase tracking-widest text-sm border border-red-400">🚪 Sair da Partida</button>}
        </div>
      </div>
    </div>
  );
  ...
  if (isUpdating) {
    return (
      <div className="fixed inset-0 bg-black/95 z-[999] flex flex-col items-center justify-center text-white font-sans">
        ...
        <h2 className="text-2xl font-black animate-pulse text-yellow-400 drop-shadow-md">Atualizando o Jogo...</h2>
      </div>
    );
  }
  ...
  if (!roomId) {
    return (
      <div className={`min-h-screen ${getTableBg(settings.tableStyle)} flex flex-col items-center justify-start pt-12 md:pt-24 pb-32 font-sans p-4 relative overflow-y-auto w-full`} translate="no">
        <div className="absolute top-6 right-4 md:right-6 flex flex-col items-center gap-3 z-50">
          <button onClick={() => setShowSettings(true)} className="text-3xl opacity-70 hover:opacity-100 hover:rotate-90 transition-all duration-300 drop-shadow-md" title="Configurações">⚙️</button>
          <button onClick={() => setShowRules(true)} className="text-2xl opacity-70 hover:opacity-100 transition-all duration-300 bg-black/40 rounded-full w-10 h-10 flex items-center justify-center border border-white/20 shadow-lg" title="Como Jogar">❓</button>
        </div>
        ...
        <div className="w-full max-w-sm flex flex-col items-center">
          <h1 className="text-6xl md:text-7xl font-extrabold mb-2 drop-shadow-2xl tracking-tighter flex items-center justify-center gap-2 notranslate">
            <span className="font-sans" style={{ color: "#facc15" }}>♦</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-600">TOCO</span>
            <span className="font-sans" style={{ color: "#eab308" }}>♣</span>
          </h1>
          <p className="text-yellow-500/80 text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase mb-4 drop-shadow-md text-center">Desenvolvido por Ryan Kilberth</p>
          <button onClick={forceUpdateGame} className="mb-8 text-xs font-bold text-gray-400 bg-black/40 px-4 py-2 rounded-full border border-white/10 hover:text-white hover:border-white/30 transition-all active:scale-95 shadow-md">🔄 Verificar Atualizações</button>

          {isNetworkOffline && <div className="w-full bg-red-600/80 text-white font-bold text-xs py-3 px-4 rounded-xl mb-4 animate-pulse text-center border border-red-400 shadow-lg">⚠️ Sem conexão. Apenas o Modo Offline está disponível.</div>}
          ...
          <div className="bg-black/40 backdrop-blur-xl p-6 rounded-3xl border border-white/10 w-full shadow-2xl mb-8 flex flex-col items-center">
            <div className="relative mb-6 cursor-pointer transform hover:scale-105 transition-transform" onClick={() => fileInputRef.current?.click()}>
               <PlayerAvatar src={avatarBase64} name={playerName} size="lg" />
               <div className="absolute bottom-0 right-0 bg-blue-600 rounded-full p-2 border-2 border-black shadow-lg">📸</div>
               <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
            </div>

            <input type="text" placeholder="Seu Nome ou Apelido" value={playerName} onChange={(e) => setPlayerName(e.target.value)} className="w-full bg-black/50 border border-white/20 text-white rounded-xl px-4 py-3 mb-4 focus:outline-none focus:border-yellow-500 text-center font-bold text-lg" />
            {errorMsg && <p className="text-red-400 text-sm mb-4 font-bold animate-pulse text-center">{errorMsg}</p>}

            <div className="flex w-full bg-black/50 rounded-xl p-1 mb-6 border border-white/10">
              <button onClick={() => setHomeTab('entrar')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${homeTab === 'entrar' ? 'bg-blue-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Entrar</button>
              <button onClick={() => setHomeTab('criar')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${homeTab === 'criar' ? 'bg-yellow-500 text-black shadow' : 'text-gray-400 hover:text-white'}`}>Criar Sala</button>
              <button onClick={() => setHomeTab('sozinho')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${homeTab === 'sozinho' ? 'bg-gray-700 text-white shadow' : 'text-gray-400 hover:text-white'}`}>Offline</button>
            </div>

            {homeTab === 'entrar' && (
              <div className="w-full flex gap-2 animate-fade-in">
                <input type="number" placeholder="PIN da Sala" value={pinInput} disabled={isNetworkOffline || isJoining} onChange={(e) => setPinInput(e.target.value)} maxLength={4} className="w-full bg-black/50 border border-white/20 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-center font-mono text-xl tracking-widest disabled:opacity-50" />
                <button onClick={joinRoom} disabled={isNetworkOffline || isJoining} className={`font-black px-6 py-3 rounded-xl shadow-lg uppercase text-sm transition-transform active:scale-95 ${isNetworkOffline || isJoining ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-500'}`}>{isJoining ? '...' : 'ENTRAR'}</button>
              </div>
            )}
            ...
            {homeTab === 'criar' && (
              <div className="w-full animate-fade-in">
                <div className="flex gap-4 w-full mb-4">
                  <button onClick={() => setSelectedMode('1v1')} className={`flex-1 py-2 rounded-xl font-black uppercase text-xs tracking-wider transition-all shadow-md ${selectedMode === '1v1' ? 'bg-yellow-500 text-black border-2 border-white' : 'bg-black/50 text-gray-400 border border-white/10 hover:text-white'}`}>1 VS 1</button>
                  <button onClick={() => setSelectedMode('2v2')} className={`flex-1 py-2 rounded-xl font-black uppercase text-xs tracking-wider transition-all shadow-md ${selectedMode === '2v2' ? 'bg-yellow-500 text-black border-2 border-white' : 'bg-black/50 text-gray-400 border border-white/10 hover:text-white'}`}>2 VS 2</button>
                </div>
                <button onClick={createRoom} disabled={isNetworkOffline || isJoining} className={`w-full font-black py-3 rounded-xl shadow-lg uppercase tracking-widest text-sm transition-transform active:scale-95 ${isNetworkOffline || isJoining ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-black hover:from-yellow-400 hover:to-yellow-500'}`}>{isJoining ? 'CRIANDO...' : 'CRIAR NOVA SALA'}</button>
              </div>
            )}
            ...
            {homeTab === 'sozinho' && (
              <div className="w-full animate-fade-in">
                ...
                <button onClick={startSinglePlayer} className="w-full bg-gradient-to-r from-blue-700 to-indigo-800 text-white font-black py-3 rounded-xl shadow-lg uppercase tracking-widest text-sm transition-transform active:scale-95 border border-blue-500 flex items-center justify-center gap-2">
  ...
  if (gameState === "lobby" && !isSinglePlayer) {
    const requiredPlayers = gameMode === "2v2" ? 4 : 2;
    return (
      <div className={`min-h-screen ${getTableBg(settings.tableStyle)} flex flex-col items-center justify-start pt-16 md:pt-24 pb-32 text-white font-sans p-4 relative overflow-y-auto w-full`} translate="no">
        <button onClick={exitGame} className="absolute top-6 left-6 text-xl md:text-2xl opacity-70 hover:opacity-100 hover:-translate-x-1 transition-all duration-300 z-50 bg-black/40 rounded-full p-3 backdrop-blur-sm border border-white/10 shadow-lg" title="Sair da Sala">⬅️</button>
        <div className="absolute top-6 right-4 md:right-6 flex flex-col items-center gap-3 z-50">
          <button onClick={() => setShowSettings(true)} className="text-3xl opacity-70 hover:opacity-100 hover:rotate-90 transition-all duration-300 drop-shadow-md" title="Configurações">⚙️</button>
        </div>
        {showSettings && <SettingsModal />}
        <h1 className="text-6xl md:text-7xl font-extrabold mb-2 drop-shadow-2xl tracking-tighter flex items-center justify-center gap-3 notranslate overflow-visible">
          <span className="text-yellow-400 font-sans" style={{ color: "#facc15" }}>♦</span>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-600">TOCO</span>
          <span className="text-yellow-500 font-sans" style={{ color: "#eab308" }}>♣</span>
        </h1>
        <div className="bg-black/60 border border-white/20 px-6 py-2 rounded-full mb-8 shadow-inner"><span className="text-yellow-400 font-black tracking-widest uppercase">Modo {gameMode}</span></div>
        <div className="bg-black/40 backdrop-blur-xl p-8 rounded-3xl border border-white/10 text-center w-full max-w-sm shadow-2xl relative overflow-visible mb-8">
          <div className="absolute -top-5 left-1/2 transform -translate-x-1/2 bg-yellow-500 text-black font-black px-6 py-2 rounded-full border-4 border-white shadow-lg text-lg flex items-center gap-2">
            PIN: <span className="font-mono text-2xl tracking-widest bg-white/30 px-2 rounded">{roomId}</span>
          </div>
          <p className="mt-6 mb-4 text-gray-300 font-bold uppercase tracking-wider text-sm">Jogadores na Mesa ({playersList.length}/{requiredPlayers})</p>
          {gameMode === "2v2" ? (
            <div className="flex justify-between gap-4 w-full mb-8">
               <div className="flex-1 flex flex-col items-center gap-2 bg-blue-900/30 p-3 rounded-xl border border-blue-500/30">
                  <h3 className="font-black text-yellow-400 text-xs mb-1">DUPLA 1</h3>
                  <LobbySlot slotNum={0} />
                  <LobbySlot slotNum={2} />
               </div>
               <div className="flex-1 flex flex-col items-center gap-2 bg-red-900/30 p-3 rounded-xl border border-red-500/30">
                  <h3 className="font-black text-red-400 text-xs mb-1">DUPLA 2</h3>
                  <LobbySlot slotNum={1} />
                  <LobbySlot slotNum={3} />
               </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              {playersList.map((p) => (
                <div key={p.id} className="bg-gradient-to-b from-blue-500 to-blue-700 px-4 py-2 rounded-xl font-bold shadow-lg border-b-4 border-blue-900 notranslate flex items-center gap-2">
                  <PlayerAvatar src={p.avatar} name={p.name} size="sm" />
                  {p.isHost && <span title="Host">👑</span>}
                  {p.name}
                </div>
          ...
          {me?.isHost && playersList.length >= requiredPlayers ? (
            <button onClick={startGameFirstTime} className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-black font-black py-4 rounded-xl hover:from-yellow-400 hover:to-yellow-500 shadow-[0_10px_20px_rgba(234,179,8,0.3)] transition-all transform hover:scale-105 active:scale-95 uppercase tracking-widest text-lg">INICIAR JOGO</button>
          ) : (
            <div className="animate-pulse text-yellow-200 text-sm font-medium bg-yellow-900/30 py-3 rounded-xl border border-yellow-500/20">
              {playersList.length < requiredPlayers ? "Aguardando mais jogadores..." : "O Host iniciará a partida..."}
            </div>
          )}
        </div>
      </div>
  ...
  const showCards = ["playing", "round_end", "announcing_trump"].includes(gameState);
  
  // --- TELA DA MESA DE JOGO ---
  return (
    <div className={`min-h-screen ${getTableBg(settings.tableStyle)} flex flex-col font-sans overflow-hidden notranslate text-white relative`} translate="no">
      <style>{`
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] w-full h-24 bg-black/30 backdrop-blur-md border-b border-white/10 shadow-2xl relative z-50">
        <div className={`flex flex-col justify-center px-3 md:px-4 border-r border-white/10 overflow-hidden ${(!is2v2 && turn === playersList[0]?.id) ? "bg-white/5" : ""}`}>
          <div className="flex justify-between items-center w-full gap-2">
            <div className="flex items-center gap-2">
               {!is2v2 ? (
                  <>
                    <PlayerAvatar src={playersList[0]?.avatar} name={playersList[0]?.name} />
                    <span className="font-bold text-base md:text-xl drop-shadow truncate text-white">{playersList[0]?.id === "bot_1" ? "Computador" : playersList[0]?.name}</span>
                  </>
               ) : (
                  <>
                    <div className="flex -space-x-2">
                      <PlayerAvatar src={myTeam[0]?.avatar} name={myTeam[0]?.name} size="sm" isTurn={turn === myTeam[0]?.id}/>
                      {myTeam[1] && <PlayerAvatar src={myTeam[1]?.avatar} name={myTeam[1]?.name} size="sm" isTurn={turn === myTeam[1]?.id}/>}
                    </div>
                    <span className="font-black text-base md:text-xl drop-shadow truncate text-yellow-400">NÓS</span>
                  </>
               )}
            </div>
          </div>
          <div className="flex items-end justify-between mt-1 pl-12">
            <span className="text-yellow-400 font-mono text-2xl md:text-3xl font-bold drop-shadow-md">{!is2v2 ? (roundScores[playersList[0]?.id] || 0) : myTeamScore}<span className="text-xs md:text-sm text-gray-400 font-sans">/31</span></span>
            <div className="flex flex-col items-end gap-0.5">
              {((!is2v2 && tocoTarget === playersList[0]?.id) || (is2v2 && myTeam.some(p => p?.id === tocoTarget))) && (
                <div className="bg-black/50 px-2 py-1 rounded-full flex gap-1 items-center mt-1 border border-white/10">
                  <span className="text-[10px] font-bold text-gray-300 mr-1">{4 - lives}ª Mão</span>
                  {[...Array(lives)].map((_, i) => (<span key={i} className="text-[10px] animate-pulse">❤️</span>))}
                </div>
              )}
              <span className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wide">
                Tocos: <span className="text-white font-bold text-xs md:text-sm bg-white/10 px-1.5 py-0.5 rounded">{!is2v2 ? (gamePoints[playersList[0]?.id] || 0) : myTeamTocos}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="w-14 md:w-16 flex flex-col items-center justify-center bg-black/60 border-x border-white/10 shadow-inner px-1">
          <span className="text-gray-500 font-black text-sm italic mb-1">VS</span>
          {is2v2 && <span className="text-[8px] text-gray-400 font-bold uppercase mt-1">2v2</span>}
        </div>

        <div className={`flex flex-col justify-center px-3 md:px-4 border-l border-white/10 overflow-hidden ${(!is2v2 && turn === playersList[1]?.id) ? "bg-white/5" : ""}`}>
          <div className="flex justify-between items-center flex-row-reverse w-full gap-2">
            <div className="flex items-center flex-row-reverse gap-2">
               {!is2v2 ? (
                  <>
                    <PlayerAvatar src={playersList[1]?.avatar} name={playersList[1]?.name} />
                    <span className="font-bold text-base md:text-xl drop-shadow truncate text-right text-white">{playersList[1]?.id === "bot_1" ? "Computador" : playersList[1]?.name}</span>
                  </>
               ) : (
                  <>
                    <div className="flex -space-x-2 flex-row-reverse space-x-reverse">
                      <PlayerAvatar src={opTeam[0]?.avatar} name={opTeam[0]?.name} size="sm" isTurn={turn === opTeam[0]?.id} isOpponent={true}/>
                      {opTeam[1] && <PlayerAvatar src={opTeam[1]?.avatar} name={opTeam[1]?.name} size="sm" isTurn={turn === opTeam[1]?.id} isOpponent={true}/>}
                    </div>
                    <span className="font-black text-base md:text-xl drop-shadow truncate text-right text-red-400">ELES</span>
                  </>
               )}
            </div>
          </div>
          <div className="flex items-end justify-between mt-1 pr-12 flex-row-reverse">
            <span className="text-yellow-400 font-mono text-2xl md:text-3xl font-bold drop-shadow-md">{!is2v2 ? (roundScores[playersList[1]?.id] || 0) : opTeamScore}<span className="text-xs md:text-sm text-gray-400 font-sans">/31</span></span>
            <div className="flex flex-col items-start gap-0.5">
              {((!is2v2 && tocoTarget === playersList[1]?.id) || (is2v2 && opTeam.some(p => p?.id === tocoTarget))) && (
                <div className="bg-black/50 px-2 py-1 rounded-full flex gap-1 items-center mt-1 border border-white/10 flex-row-reverse">
                  <span className="text-[10px] font-bold text-gray-300 ml-1">{4 - lives}ª Mão</span>
                  {[...Array(lives)].map((_, i) => (<span key={i} className="text-[10px] animate-pulse">❤️</span>))}
                </div>
              )}
              <span className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wide">
                Tocos: <span className="text-white font-bold text-xs md:text-sm bg-white/10 px-1.5 py-0.5 rounded">{!is2v2 ? (gamePoints[playersList[1]?.id] || 0) : opTeamTocos}</span>
              </span>
            </div>
          </div>
        </div>
      ...
      {gameState === "announcing_trump" && trumpSuit && (
        <div className="absolute inset-0 bg-black/85 backdrop-blur-sm z-[200] flex flex-col items-center justify-center animate-fade-in pointer-events-auto">
          <div className="flex flex-col items-center gap-6">
            <div className="flex items-center gap-3 bg-white/10 px-6 py-3 rounded-full border border-white/20 shadow-lg">
              <PlayerAvatar src={playersList.find(p => p?.id === tocoTarget)?.avatar} name={playersList.find(p => p?.id === tocoTarget)?.name} size="sm" />
              <span className="text-white font-bold uppercase tracking-widest text-sm md:text-base">
                {playersList.find(p => p?.id === tocoTarget)?.name || "Alguém"} escolheu o trunfo:
              </span>
            </div>
            <div className="flex items-center justify-center gap-6 bg-gradient-to-b from-gray-800 to-black p-8 rounded-3xl border-2 border-yellow-500 shadow-[0_0_80px_rgba(234,179,8,0.5)] transform scale-110">
              <div className="w-20 h-20 md:w-28 md:h-28 bg-white rounded-full flex items-center justify-center shadow-inner">
                <span className={`text-6xl md:text-8xl font-sans ${SUITS[trumpSuit]?.defaultColor || "text-white"}`}>{SUITS[trumpSuit]?.symbol || "♠"}</span>
              </div>
              <span className="text-4xl md:text-6xl font-black text-yellow-400 uppercase tracking-widest drop-shadow-lg">{SUITS[trumpSuit]?.name || "Desconhecido"}</span>
            </div>
            <div className="mt-6 flex flex-col items-center gap-2">
              <div className="flex gap-1 text-2xl">{[...Array(lives)].map((_, i) => (<span key={i} className="animate-pulse">❤️</span>))}</div>
              <span className="text-yellow-400 font-bold uppercase tracking-widest text-sm">{4 - lives}ª Mão do Toco</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="absolute top-28 right-2 md:right-4 flex flex-col gap-3 z-50">
        <button onClick={() => setShowSettings(true)} className="text-2xl opacity-60 hover:opacity-100 transition-all duration-300 bg-black/40 rounded-full w-10 h-10 flex items-center justify-center backdrop-blur-sm border border-white/10 shadow-lg" title="Configurações">⚙️</button>
        <button onClick={() => setShowRules(true)} className="text-xl opacity-60 hover:opacity-100 transition-all duration-300 bg-black/40 rounded-full w-10 h-10 flex items-center justify-center backdrop-blur-sm border border-white/10 shadow-lg font-bold" title="Como Jogar">❓</button>
      </div>
      ...
      {currentHint && (
        <div className="absolute bottom-40 md:bottom-32 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm pointer-events-none">
          <div className="bg-blue-900/95 backdrop-blur-md border-2 border-blue-400 p-4 rounded-2xl shadow-2xl animate-fade-in text-center relative pointer-events-auto">
            <button onClick={() => setCurrentHint(null)} className="absolute -top-2 -right-2 bg-red-500 w-6 h-6 rounded-full text-xs font-bold shadow border border-white z-50 cursor-pointer">X</button>
            <p className="text-sm md:text-base font-medium text-white leading-tight">{currentHint.text}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative w-full mt-4">
        ...
        {!is2v2 && showCards && (
          <div className="absolute top-8 md:top-12 flex -space-x-4 md:-space-x-6 transition-all duration-500 hover:-space-x-2 z-10">
            {Array.from({ length: hands[playersList.find(p => p?.id !== me?.id)?.id]?.length || 0 }).map((_, i) => (<CardBack key={i} settings={settings} />))}
          </div>
        )}

        {/* RENDERIZAÇÃO DOS OPONENTES E PARCEIRO NA BORDA DA TELA (2v2) */}
        {is2v2 && showCards && topPlayer && (
           <EdgePlayer player={topPlayer} position="top" handCount={hands[topPlayer.id]?.length || 0} isTurn={turn === topPlayer.id} isOpponent={false} tocoTarget={tocoTarget} lives={lives} settings={settings} />
        )}
        {is2v2 && showCards && leftPlayer && (
           <EdgePlayer player={leftPlayer} position="left" handCount={hands[leftPlayer.id]?.length || 0} isTurn={turn === leftPlayer.id} isOpponent={true} tocoTarget={tocoTarget} lives={lives} settings={settings} />
        )}
        {is2v2 && showCards && rightPlayer && (
           <EdgePlayer player={rightPlayer} position="right" handCount={hands[rightPlayer.id]?.length || 0} isTurn={turn === rightPlayer.id} isOpponent={true} tocoTarget={tocoTarget} lives={lives} settings={settings} />
        )}

        {/* ÁREA CENTRAL: Baralho e Trunfo */}
        <div className={`absolute ${is2v2 ? 'left-4 md:left-12 top-[15%] md:top-[20%]' : 'left-4 md:left-8 top-1/2 -translate-y-1/2'} flex flex-col items-center opacity-80 hover:opacity-100 transition-all z-0`}>
          {deck.length > 0 && (
            <div className={`${is2v2 ? 'w-12 h-16 md:w-16 md:h-24' : 'w-[52px] h-[76px] md:w-20 md:h-28'} bg-blue-900 border-2 border-white/50 rounded-lg flex items-center justify-center shadow-xl relative mt-2 md:mt-4`} style={{ backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 8px, rgba(255,255,255,0.08) 8px, rgba(255,255,255,0.08) 16px)", }}>
              <div className="absolute inset-1 border border-white/30 rounded border-dashed"></div>
              <div className="absolute -bottom-2 -right-2 bg-red-600 text-[10px] text-white font-bold rounded-full w-5 h-5 flex items-center justify-center border-2 border-white shadow">{deck.length}</div>
              {trumpSuit && (
                <div className={`absolute -top-4 -right-4 md:-top-6 md:-right-6 ${is2v2 ? 'w-8 h-8 md:w-12 md:h-12 border-2 md:border-4' : 'w-10 h-10 md:w-14 md:h-14 border-4'} bg-white rounded-full border-yellow-500 flex items-center justify-center ${is2v2 ? 'text-xl md:text-3xl' : 'text-xl md:text-3xl'} shadow-[0_0_20px_rgba(250,204,21,0.8)] transition-transform hover:scale-110 z-10`}>
                  <span className={`${SUITS[trumpSuit]?.defaultColor || "text-white"} font-sans`}>{SUITS[trumpSuit]?.symbol || "♠"}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className={`relative flex flex-col items-center justify-center w-full ${is2v2 ? 'translate-y-12 md:translate-y-16' : 'translate-y-6 md:translate-y-12'} ${isShaking ? 'animate-shake' : ''}`}>
          <div className={`${is2v2 ? 'grid grid-cols-2 gap-x-8 gap-y-4' : 'flex gap-4 md:gap-8 flex-wrap'} items-center justify-items-center min-h-[160px] z-10 px-4`}>
            {tableCards.map((tc) => {
              ...
              let animationClass = "";
              if (sweepingTo) {
                animationClass = isAnimEnabled ? (isMyTeamSweep ? "animate-sweep-left" : "animate-sweep-right") : "opacity-0";
              } else if (isAnimEnabled) {
                animationClass = tc.isHeavy ? "animate-heavy-drop" : "animate-deal-table";
              }

              return (
                <div key={tc.card.id} className={`flex flex-col items-center ${animationClass}`}>
                  <CardFace card={tc.card} playable={false} settings={settings} trumpSuit={trumpSuit} />
                  <span className="bg-black/60 backdrop-blur text-white text-[10px] md:text-xs px-3 md:px-4 py-1 rounded-full mt-3 font-bold shadow-lg border border-white/20">
                    {playersList.find((p) => p?.id === tc.playerId)?.name || "Robô"}
                  </span>
                </div>
              );
            })}
          ...
          {trickFeedback && (
            <div className="absolute z-30 bg-white/95 backdrop-blur px-8 py-4 rounded-2xl border-4 border-yellow-500 shadow-[0_0_50px_rgba(234,179,8,0.6)] animate-fade-in text-center transform scale-110">
              ...
              <span className="inline-block bg-green-100 text-green-700 font-extrabold px-3 py-1 rounded-full text-base md:text-lg border border-green-300">+{trickFeedback.pts} pts</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gradient-to-t from-black/95 to-transparent pb-8 pt-4 w-full flex flex-col items-center relative z-10">
        {showCards && (
          <>
            <div className={`absolute left-4 ${is2v2 ? 'bottom-44 md:bottom-32 gap-6' : 'bottom-48 md:bottom-20 gap-2'} z-40 flex flex-col items-center`}>
              <button onClick={() => setShowHistory(true)} className="text-white/60 hover:text-white transition-all duration-200 flex flex-col items-center gap-1 active:scale-95">
                <CardFanIcon />
                <span className="text-[10px] md:text-xs font-bold tracking-wider">Mãos ({trickHistory.length})</span>
              </button>
              {is2v2 && (
                <div className="flex flex-col items-center gap-1">
                  <PlayerAvatar src={me?.avatar} name={playerName} size="md" isTurn={turn === me?.id} isOpponent={false} />
                </div>
              )}
            </div>

            {turn === me?.id && settings.showHints && gameState !== "announcing_trump" && (
              <div className={`absolute right-4 ${!isSinglePlayer ? "bottom-[260px] md:bottom-36" : "bottom-32 md:bottom-20"} z-40 transition-all`}>
                <button onClick={generateHint} className="text-yellow-400 hover:text-yellow-300 transition-all duration-200 flex flex-col items-center gap-1 active:scale-95 animate-pulse" title="Pedir uma dica">
                  <span className="text-2xl md:text-3xl drop-shadow-lg opacity-90">💡</span>
                  <span className="text-[10px] md:text-xs font-bold tracking-wider text-yellow-400">Dica</span>
                </button>
              </div>
            )}
          </>
        ...
        <div className="mb-4 h-10 flex items-center justify-center">
          {gameState === "announcing_trump" ? null : turn === me?.id ? (
            <span className="bg-yellow-400 text-black font-black px-8 py-2 md:py-3 rounded-full animate-pulse shadow-[0_0_25px_rgba(250,204,21,0.5)] border-2 border-white tracking-widest text-sm md:text-base uppercase cursor-default">SUA VEZ DE JOGAR</span>
          ) : (
            <span className="text-gray-300 text-xs md:text-sm bg-black/50 backdrop-blur px-6 py-2 rounded-full flex items-center gap-2 border border-white/10"><div className="w-2 h-2 bg-yellow-500 rounded-full animate-ping"></div> Aguardando jogada...</span>
          )}
        </div>

        <div className="flex -space-x-3 md:space-x-4 px-4 h-32 md:h-44 items-end pb-2 overflow-visible">
          {showCards && (hands[me?.id] || []).map((card, index) => (
              <div 
                key={card.id} 
                className={`transition-transform duration-200 hover:-translate-y-6 hover:z-20 overflow-visible ${(settings.animations ?? true) ? 'animate-deal-up' : ''}`}
                style={{ animationDelay: `${index * 0.1}s`, animationFillMode: 'backwards' }}
              >
                <CardFace card={card} playable={turn === me?.id && gameState === "playing"} onClick={handleCardClick} isHinted={currentHint?.cardId === card.id} settings={settings} trumpSuit={trumpSuit} localProcessing={localProcessing} />
              </div>
            ))}
        </div>
      </div>
          ...
          {tocoTarget === me?.id ? (
            <div className="bg-white rounded-3xl p-8 text-center shadow-[0_0_50px_rgba(250,204,21,0.4)] max-w-sm w-full border-4 border-yellow-500 relative overflow-visible">
              ...
              <div className="grid grid-cols-2 gap-4 overflow-visible">
                {Object.keys(SUITS).map((s) => (
                  <button key={s} onClick={() => confirmTrumpAndDeal(s)} className="group border-2 border-gray-100 p-6 rounded-2xl hover:bg-yellow-50 hover:border-yellow-400 flex flex-col items-center transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md bg-gray-50 overflow-visible">
                    <span className={`${SUITS[s].defaultColor} text-5xl mb-3 group-hover:scale-125 transition-transform duration-200 font-sans`}>{SUITS[s].symbol}</span>
            ...
            <div className="text-center text-white bg-black/60 p-8 rounded-3xl border border-white/20 backdrop-blur-md shadow-2xl">
              ...
              <h2 className="text-2xl md:text-3xl font-black text-yellow-400 mb-2 drop-shadow">
                Aguardando {playersList.find(p => p?.id === tocoTarget)?.name || "Alguém"}...
              </h2>
              <p className="text-gray-300 text-sm font-medium">O jogador está escolhendo o naipe de corte.</p>
            </div>
          )}
        </div>
      )}
      ...
      {showHistory && (
        <div className="absolute inset-0 bg-black/95 z-[60] flex flex-col items-center p-6 overflow-y-auto backdrop-blur-md">
          <div className="w-full max-w-md flex justify-between items-center mb-6 mt-4 z-10">
            <h2 className="text-3xl font-black text-yellow-400 drop-shadow">Mãos {is2v2 ? "do Time" : "da Rodada"} 🗂️</h2>
          ...
          <div className="w-full max-w-md flex flex-col gap-4 pb-10">
            {trickHistory.length === 0 ? (
              <div className="bg-white/5 border border-white/10 p-8 rounded-2xl text-center shadow-inner"><p className="text-gray-400 font-medium">{is2v2 ? "Seu time" : "Você"} ainda não levou nenhuma mão.</p></div>
            ) : (
              trickHistory.map((trick, index) => (
                <div key={trick.id} className="bg-white/10 p-4 rounded-xl border border-white/20 flex flex-col items-center relative shadow-lg">
                  ...
                  <div className="flex gap-4">{trick.cards.map((c) => (<MiniCard key={c.id} card={c} settings={settings} />))}</div>
                </div>
              ))
            )}
          </div>
        ...
        <div className="absolute inset-0 bg-black/90 z-[70] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-gradient-to-br from-gray-900 to-black p-1 rounded-3xl shadow-[0_0_80px_rgba(234,179,8,0.4)] max-w-sm w-full border border-gray-700">
            <div className="bg-gray-900/50 backdrop-blur-md p-10 rounded-[22px] text-center overflow-visible">
              <div className="text-2xl md:text-3xl font-black text-white mb-8 uppercase flex flex-col gap-3 leading-tight tracking-wide drop-shadow-md overflow-visible">{getEndGameMessage()}</div>
              {me?.isHost ? (
                <button onClick={() => startNewHand(tocoTarget)} className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-black font-extrabold py-5 px-8 rounded-xl hover:from-yellow-400 hover:to-yellow-500 shadow-[0_10px_20px_rgba(234,179,8,0.3)] uppercase tracking-widest transition-all transform hover:scale-105 active:scale-95 text-lg">Próxima Mão</button>
              ) : (
...
export default function App() {
  ...
  const myTeam = is2v2
    ...
    : [playersList[myIdx]].filter(Boolean);
  const opTeam = is2v2
    ? [playersList[(myIdx + 1) % 4], playersList[(myIdx + 3) % 4]].filter(
        Boolean
      )
    : [playersList[(myIdx + 1) % 2]].filter(Boolean);
      ...
      if (isMyTeamWinner)
        return (
          <span className="text-green-400 drop-shadow-md">
            O ALVO ({tocoTargetName}) SE LIVROU! 😅
          </span>
        );
      else
        return (
          <span className="text-yellow-400 drop-shadow-md">
            ELES ESCAPARAM! O TOCO É {is2v2 ? "DE VOCÊS" : "SEU"}! 🫵
          </span>
        );
    }
    if (type === "life_lost") {
  ...
  const LobbySlot = ({ slotNum }) => {
    ...
    return (
      <button
        ...
        disabled={isJoining}
        className="bg-white/5 border border-dashed border-white/20 px-3 py-3 rounded-lg text-[10px] uppercase font-bold text-gray-400 hover:text-white hover:bg-white/10 w-full transition-colors flex justify-center disabled:opacity-50"
      >
        Sentar Aqui
      </button>
    );
            ...
            {homeTab === "entrar" && (
              <div className="w-full flex gap-2 animate-fade-in">
                <input
                  ...
                  value={pinInput}
                  disabled={isNetworkOffline || isJoining}
  ...
  const showCards = ["playing", "round_end", "announcing_trump"].includes(
    gameState
  );
          ...
          {showCards &&
            (hands[me?.id] || []).map((card, index) => (
              ...
              >
                <CardFace
                  card={card}
                  playable={turn === me?.id && gameState === "playing"}
