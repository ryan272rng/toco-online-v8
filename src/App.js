...
const PlayerAvatar = ({ src, name, size = "md", isTurn, isOpponent }) => {
  const dim = size === "lg" ? "w-24 h-24 text-3xl" : size === "sm" ? "w-8 h-8 text-sm" : "w-10 h-10 md:w-12 md:h-12 text-lg";
  ...
  if (isTurn !== undefined) {
    ringColor = isTurn ? (isOpponent ? "ring-4 ring-red-500 shadow-[0_0_15px_rgba(239,68,68,0.8)]" : "ring-4 ring-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.8)]") : "border-2 border-white/30 shadow-lg";
  }

  if (src) {
    return <img src={src} alt={name} className={`${dim} rounded-full object-cover ${ringColor} transition-all duration-300`} />;
  }
  return (
    <div className={`${dim} rounded-full bg-gradient-to-br from-blue-700 to-indigo-900 ${ringColor} flex items-center justify-center text-white font-black transition-all duration-300`}>
...
const EdgePlayer = ({ player, handCount, position, isTurn, isOpponent, tocoTarget, lives, settings }) => {
  ...
  let containerClass = "absolute flex flex-col items-center z-10 transition-all ";
  ...
  return (
    <div className={containerClass}>
        ...
        <div className="bg-black/50 backdrop-blur rounded px-2 py-0.5 text-white/90 flex flex-col items-center gap-0.5 shadow-md border border-white/10">
          ...
          {isTarget && (
             <div className="flex gap-0.5">
               {[...Array(lives)].map((_, i) => (<span key={i} className="text-[6px]">❤️</span>))}
             </div>
          )}
        </div>
      </div>
      ...
      {handCount > 0 && (
        <div className={`flex flex-row -space-x-3 md:-space-x-4 transition-all duration-300 opacity-90 hover:opacity-100`}>
...
export default function App() {
  ...
  const [settings, setSettings] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { sound: true, vibration: true, showHints: true, saveName: false, deckStyle: "default", cardSize: "normal", animations: true, tableStyle: "tradicional" };
  });
  ...
  const createRoom = async () => {
    ...
    const newMe = { id: myId, name: playerName, isHost: true, avatar: avatarBase64 };
  ...
  const joinRoom = async () => {
    ...
    if (snapshot.exists()) {
      ...
      if (matchedPlayer) {
        ...
        newMe = { id: myId, name: playerName, isHost: false, avatar: avatarBase64 };
  ...
  const startSinglePlayer = () => {
    ...
    const bots = {};
    if (selectedMode === "2v2") {
      bots["bot_1"] = { id: "bot_1", name: "Robô Esquerda", isHost: false, avatar: "" };
      bots["bot_2"] = { id: "bot_2", name: "Robô Aliado", isHost: false, avatar: "" };
      bots["bot_3"] = { id: "bot_3", name: "Robô Direita", isHost: false, avatar: "" };
    } else {
      bots["bot_1"] = { id: "bot_1", name: "Computador", isHost: false, avatar: "" };
    }
  ...
  const myTeam = is2v2 ? [playersList[myIdx], playersList[(myIdx + 2) % 4]].filter(Boolean) : [playersList[myIdx]];
  const opTeam = is2v2 ? [playersList[(myIdx + 1) % 4], playersList[(myIdx + 3) % 4]].filter(Boolean) : [playersList[(myIdx + 1) % 2]];
  ...
  useEffect(() => {
    if (me?.isHost && gameState === "dealing" && roomData) {
      ...
      syncState({ hands: newHands, deck: remaining, turn: roomData.tocoTarget, gameState: "playing" });
    }
  }, [gameState, me?.isHost]);
  ...
  const handleGameEnd = (winningTeam) => {
    ...
    else { playSoundEffect("lose"); if (settings.vibration && navigator.vibrate) navigator.vibrate([100, 50, 100]); }
  ...
  const getEndGameMessage = () => {
    ...
    if (type === "escaped") {
      if (isMyTeamWinner) return <span className="text-green-400 drop-shadow-md">{is2v2 ? 'SEU TIME' : 'VOCÊ'} SE LIVROU! 😅</span>;
      else return <span className="text-yellow-400 drop-shadow-md">{is2v2 ? 'ELES ESCAPARAM' : 'ELE ESCAPOU'}! O TOCO É {is2v2 ? 'DE VOCÊS' : 'SEU'}! 🫵</span>;
    }
    if (type === "life_lost") {
      if (!isMyTeamWinner) return <span className="text-red-400 drop-shadow-md">{is2v2 ? 'SEU TIME' : 'VOCÊ'} PERDEU 1 VIDA! 💔</span>;
      else return <span className="text-green-400 drop-shadow-md">VOCÊS TIRARAM UMA VIDA DELES! ⚔️</span>;
    }
    if (type === "toco_confirmed") {
      if (!isMyTeamWinner) return <span className="text-red-600 drop-shadow-md">QUE PENA! {is2v2 ? 'SEU TIME' : 'VOCÊ'} PEGOU O TOCO. 🪵</span>;
    ...
    <div className="absolute inset-0 bg-black/90 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-gray-900 border border-yellow-500/30 rounded-3xl w-full max-w-sm p-6 shadow-[0_0_50px_rgba(234,179,8,0.2)]">
        ...
        <div className="flex flex-col gap-4">
          ...
          <button onClick={forceUpdateGame} className="w-full bg-blue-600/90 text-white font-bold py-2 rounded-xl hover:bg-blue-500 transition-colors flex items-center justify-center gap-2">
          ...
          <label className="flex justify-between items-center text-white font-medium"><span>📳 Vibração (Tátil)</span><input type="checkbox" checked={settings.vibration} onChange={() => toggleSetting("vibration")} className="w-6 h-6 accent-yellow-500" /></label>
    ...
    return (
      <div className={`min-h-screen ${getTableBg(settings.tableStyle)} flex flex-col items-center justify-start pt-12 md:pt-24 pb-32 font-sans p-4 relative overflow-y-auto w-full`} translate="no">
        ...
        <div className="w-full max-w-sm flex flex-col items-center">
          ...
          <p className="text-yellow-500/80 text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase mb-10 drop-shadow-md text-center">Desenvolvido por Ryan Kilberth</p>
          ...
          <div className="bg-black/40 backdrop-blur-xl p-8 rounded-3xl border border-white/10 w-full shadow-2xl mb-8 flex flex-col items-center">
            ...
            <input type="text" placeholder="Seu Nome ou Apelido" value={playerName} onChange={(e) => setPlayerName(e.target.value)} className="w-full bg-black/50 border border-white/20 text-white rounded-xl px-4 py-4 mb-6 focus:outline-none focus:border-yellow-500 text-center font-bold text-lg" />
            {errorMsg && <p className="text-red-400 text-sm mb-4 font-bold animate-pulse text-center">{errorMsg}</p>}

            <div className="flex gap-4 mb-6 w-full">
               <button onClick={() => setSelectedMode('1v1')} className={`flex-1 py-3 rounded-xl font-black uppercase text-sm tracking-wider transition-all shadow-md ${selectedMode === '1v1' ? 'bg-yellow-500 text-black border-2 border-white' : 'bg-black/50 text-gray-400 border border-white/10 hover:text-white'}`}>1 VS 1</button>
               <button onClick={() => setSelectedMode('2v2')} className={`flex-1 py-3 rounded-xl font-black uppercase text-sm tracking-wider transition-all shadow-md ${selectedMode === '2v2' ? 'bg-yellow-500 text-black border-2 border-white' : 'bg-black/50 text-gray-400 border border-white/10 hover:text-white'}`}>2 VS 2</button>
            </div>

            <button onClick={startSinglePlayer} className="w-full bg-gradient-to-r from-blue-700 to-indigo-800 text-white font-black py-4 rounded-xl shadow-lg mb-6 uppercase tracking-widest text-sm transition-transform active:scale-95 border border-blue-500 flex items-center justify-center gap-2">
              <span className="text-xl">🤖</span> JOGAR SOZINHO
            </button>

            <div className="flex items-center gap-2 mb-6 opacity-60">
              <div className="h-px bg-white/20 flex-1"></div>
              <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Multiplayer</span>
              <div className="h-px bg-white/20 flex-1"></div>
            </div>

            <button onClick={createRoom} disabled={isNetworkOffline} className={`w-full font-black py-3 rounded-xl shadow-lg mb-4 uppercase tracking-widest text-sm transition-transform active:scale-95 ${isNetworkOffline ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-black hover:from-yellow-400 hover:to-yellow-500'}`}>CRIAR NOVA SALA</button>
            <div className="flex gap-2">
              <input type="number" placeholder="PIN" value={pinInput} disabled={isNetworkOffline} onChange={(e) => setPinInput(e.target.value)} maxLength={4} className="w-full bg-black/50 border border-white/20 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 text-center font-mono text-xl tracking-widest disabled:opacity-50" />
  ...
  const myTricks = trickHistory.filter((t) => myTeam.some(p => p.id === t.winnerId));

  // --- TELA DA MESA DE JOGO ---
  return (
    <div className={`min-h-screen ${getTableBg(settings.tableStyle)} flex flex-col font-sans overflow-hidden notranslate text-white relative ${isShaking ? 'animate-shake' : ''}`} translate="no">
      ...
      {activeChatMessage && (
         <div className="pointer-events-none absolute top-[100px] left-1/2 -translate-x-1/2 z-[140] bg-white text-blue-900 font-black text-sm px-6 py-3 rounded-3xl shadow-2xl border-2 border-gray-200 animate-chat whitespace-nowrap">
           <span className="text-gray-500 mr-2 text-xs">{playersList.find(p => p.id === activeChatMessage.senderId)?.name}:</span>
      ...
      {currentHint && (
        <div className="absolute bottom-64 md:bottom-56 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-sm">
          <div className="bg-blue-900/95 backdrop-blur-md border-2 border-blue-400 p-4 rounded-2xl shadow-2xl animate-fade-in text-center relative">
            <button onClick={() => setCurrentHint(null)} className="absolute -top-2 -right-2 bg-red-500 w-6 h-6 rounded-full text-xs font-bold shadow border border-white z-50">X</button>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative w-full mt-4">
        ...
        <div className={`absolute ${is2v2 ? 'left-4 md:left-12 top-[25%] md:top-[30%]' : 'left-4 md:left-8 top-1/2 -translate-y-1/2'} flex flex-col items-center gap-4 opacity-80 hover:opacity-100 transition-all`}>
          {deck.length > 0 && (
            <div className={`${is2v2 ? 'w-12 h-16 md:w-16 md:h-24' : 'w-[52px] h-[76px] md:w-20 md:h-28'} bg-blue-900 border-2 border-white/50 rounded-lg flex items-center justify-center shadow-xl relative`} style={{ backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 8px, rgba(255,255,255,0.08) 8px, rgba(255,255,255,0.08) 16px)", }}>
              <div className="absolute inset-1 border border-white/30 rounded border-dashed"></div>
              <div className="absolute -top-2 -right-2 bg-red-600 text-[10px] text-white font-bold rounded-full w-5 h-5 flex items-center justify-center border-2 border-white shadow">{deck.length}</div>
            </div>
          )}
          {trumpSuit && (
            <div className={`${is2v2 ? 'w-8 h-8 md:w-12 md:h-12 border-2' : 'w-10 h-10 md:w-14 md:h-14 border-4'} bg-white rounded-full border-yellow-500 flex items-center justify-center ${is2v2 ? 'text-lg md:text-2xl' : 'text-xl md:text-3xl'} shadow-[0_0_15px_rgba(250,204,21,0.5)]`}>
        ...
        <div className="relative flex flex-col items-center justify-center w-full translate-y-6 md:translate-y-12">
          <div className="flex gap-4 md:gap-8 items-center h-40 md:h-48 z-10 flex-wrap justify-center px-4">
            {tableCards.map((tc) => {
              ...
              const isLeft = sweepingTo === playersList[0]?.id || sweepingTo === leftPlayer?.id;
              const isAnimEnabled = settings.animations ?? true;
              
              let animationClass = "";
              if (sweepingTo) {
                animationClass = isAnimEnabled ? (isLeft ? "animate-sweep-left" : "animate-sweep-right") : "opacity-0";
      ...
      <div className="bg-gradient-to-t from-black/95 to-transparent pb-8 pt-4 w-full flex flex-col items-center relative z-10">
        ...
        {is2v2 && (
          <div className="absolute left-4 bottom-48 md:bottom-20 z-40 flex flex-col items-center gap-1">
            <PlayerAvatar src={me?.avatar} name={playerName} size="sm" isTurn={turn === me?.id} isOpponent={false} />
            {tocoTarget === me?.id && (
               <div className="flex gap-0.5 mt-0.5 justify-center">
                 {[...Array(lives)].map((_, i) => (<span key={i} className="text-[8px]">❤️</span>))}
               </div>
            )}
          </div>
        )}
        ...
        {showCards && (
          <>
            <div className={`absolute ${is2v2 ? 'left-16' : 'left-4'} bottom-48 md:bottom-20 z-40`}>
            ...
            {!isSinglePlayer && (
              <div className="absolute right-4 bottom-48 md:bottom-20 z-40 flex flex-col items-end gap-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setShowChat(!showChat); setShowEmotes(false); }} className="text-2xl opacity-80 hover:opacity-100 transition-opacity active:scale-90 bg-black/40 rounded-full w-12 h-12 flex items-center justify-center border border-white/10 shadow-lg" title="Chat Rápido">💬</button>
                  <button onClick={() => { setShowEmotes(!showEmotes); setShowChat(false); }} className="text-3xl opacity-80 hover:opacity-100 transition-opacity active:scale-90 bg-black/40 rounded-full w-12 h-12 flex items-center justify-center border border-white/10 shadow-lg" title="Reagir">😀</button>
                </div>
              </div>
            )}
        ...
        <div className="flex -space-x-3 md:space-x-4 px-4 h-32 md:h-44 items-end pb-2 overflow-visible">
          {showCards && myHand.map((card, index) => (
              ...
              >
                <CardFace card={card} playable={turn === me?.id} onClick={handleCardClick} isHinted={currentHint?.cardId === card.id} settings={settings} trumpSuit={trumpSuit} localProcessing={localProcessing} />
              </div>
            ))}
        </div>
      </div>
        ...
        <div className="absolute inset-0 bg-black/90 z-[70] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-gradient-to-br from-gray-900 to-black p-1 rounded-3xl shadow-[0_0_80px_rgba(234,179,8,0.4)] max-w-sm w-full border border-gray-700">
            <div className="bg-gray-900/50 backdrop-blur-md p-10 rounded-[22px] text-center overflow-visible">
              <div className="text-3xl font-black text-white mb-8 uppercase flex flex-col gap-3 leading-tight tracking-wide drop-shadow-md overflow-visible">{getEndGameMessage()}</div>
              {me?.isHost ? (
